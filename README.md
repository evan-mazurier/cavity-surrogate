...
<p align="center"><img src="figures/cavity.gif" width="420" alt="fluid particles carried by the surrogate's predicted velocity field, Re sweeping 100 to 1000"></p>
<p align="center"><b><a href="https://evan-mazurier.github.io/cavity-surrogate/">Open the interactive demo</a></b> — drag the Reynolds number, watch the fluid move, check the prediction against the solver.</p>
...
## The demo — `docs/` → [evan-mazurier.github.io/cavity-surrogate](https://evan-mazurier.github.io/cavity-surrogate/)

A single static page, served by GitHub Pages. A Re slider on a log scale with the training range shaded; the
predicted field drawn live (~1 ms per prediction — mean + 20 modes, evaluated in the browser) as a speed map with
streamlines and **fluid particles carried by the predicted velocity** — the flow is steady, the fluid moves through it,
and every particle path is what the field says. A "check against the solver" card always shows the nearest Re the
model never trained on: the solver's real step count and wall time beside the surrogate's, the speed-up factor
(40,000–400,000×), the two fields and their difference, the relative error in plain words, and the error-vs-Re curve.
Push the slider past 1500 and watch the error climb — that is the point.
...
docs/          the interactive demo, served by GitHub Pages (model/ is generated by surrogate/export_web.py)
```
